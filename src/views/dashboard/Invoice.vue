        ...
        <div class="columns is-multiline">
            <div class="column is-12">
                ...
                <div class="buttons">
                    ...
                    <template v-if="!invoice.is_credit_for && !invoice.is_credited">
                        <button @click="setAsPaid()" class="button is-success is-outlined" v-if="!invoice.is_paid">Set as paid</button>
                        <button @click="createCreditNote()" class="button is-danger is-outlined" v-if="!invoice.is_paid">Create credit note</button>
                    </template>

                    <button @click="sendReminder()" class="button is-info is-outlined" v-if="!invoice.is_paid && !invoice.is_credit_for">Send reminder</button>
                </div>
            </div>
            ...
            <div class="column is-12 mb-4">
                <div class="box">
                    ...
                    <p v-if="invoice.client_address_1">{{ invoice.client_address_1 }}</p>
                    <p v-if="invoice.client_address_2">{{ invoice.client_address_2 }}</p>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            sendReminder() {
                axios
                    .post(`/api/v1/invoices/${this.invoice.id}/send_reminder/`)
                    .then(response => {
                        toast({
